...
import {abilityKeyFromName} from "./csAbilities.js";
        ...
        const data = {
            ...
            buttons: {
                normal: {
                    ...
                    callback: (html) => {
                        // Dialog hands over jQuery today; tolerate a plain
                        // element too. Fall back to the dialog root so a
                        // dropped <form> tag cannot lose the fields.
                        const root = html instanceof HTMLElement ? html : html[0];
                        resolve({data: root.querySelector("form") ?? root});
                    }
                },
                cancel: {
...
/**
 * Reads the roll-modifier dialog back into a formula. Fields are looked up by
 * name rather than through form.elements so it works whether or not the inner
 * <form> element survived parsing, and any field the dialog did not supply
 * falls back to the value the roll started with instead of becoming NaN.
 */
function _readModifierForm(root, fallback) {
    const formula = new DiceRollFormula();

    const read = (name, fallbackValue) => {
        const field = root ? root.querySelector(`[name="${name}"]`) : null;
        const value = field ? parseInt(field.value, 10) : NaN;
        return Number.isNaN(value) ? fallbackValue : value;
    };

    formula.pool = read("pool", fallback.pool);
    formula.bonusDice = read("bonusDice", fallback.bonusDice);
    formula.reRoll = read("reRoll", fallback.reRoll);
    formula.modifier = read("modifier", fallback.modifier);
    formula.dicePenalty = read("dicePenalty", fallback.dicePenalty);
...
async function handleRollAsync(rollType, actor, showModifierDialog = false) {
    ...
    if (showModifierDialog ? !revertModifierDialog : revertModifierDialog) {
        const form = await _showModifierDialog(formula);

        // Dismissed: no roll. The old code returned a promise that never
        // resolved here, which hung the await silently - no dice, no error.
        if (!form || form.cancelled) return null;

        const formulaChanged = _readModifierForm(form.data, formula);
...
function getActorTestFormula(actor, abilityName, specialtyName = null) {
    ...
    if (ability !== undefined) {
        ...
        // lookup must use the canonical key, not the translated item name.
        const penaltyType = abilityKeyFromName(ability.name) ?? ability.name.toLowerCase();
...
// A character may take at most two levels of stress in an intrigue. The
// sheet used to hard-code this in the template while the handler clamped
// against the frustration maximum instead, so the two could disagree.
ChronicleSystem.maxStress = 2;
...
ChronicleSystem.keyConstants = {
    ...
    ANIMAL_HANDLING: "CS.constants.abilities.animalHandling",
    ...
    FIGHTING: "CS.constants.abilities.fighting",
    HEALING: "CS.constants.abilities.healing",
    KNOWLEDGE: "CS.constants.abilities.knowledge",
    LANGUAGE: "CS.constants.abilities.language",
    ...
    STEALTH: "CS.constants.abilities.stealth",
    SURVIVAL: "CS.constants.abilities.survival",
    THIEVERY: "CS.constants.abilities.thievery",
    WARFARE: "CS.constants.abilities.warfare",
