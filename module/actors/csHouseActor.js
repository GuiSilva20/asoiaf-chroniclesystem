...
export class CSHouseActor extends CSActor {
    ...
    async removeCharacterFromHouse(
        actorId,
        role = undefined,
        ignoreRoles = ['STEWARD']
    ) {
        if (role) {
            ...
            if (result.hasRole)
                await this._removeCharacterFromRole(
                    role,
                    actorId,
                    result.index
                );
        } else {
            // sequential: each removal writes the whole member list back, so a
            // forEach firing them in parallel would drop all but the last write
            for (const roleKey of Object.keys(this.roleMap)) {
                if (ignoreRoles.includes(roleKey)) continue;
                let result = this.characterHasRole(actorId, roleKey);
                if (result.hasRole)
                    await this._removeCharacterFromRole(
                        roleKey,
                        actorId,
    ...
    async _removeCharacterFromRole(role, actorId, index = -1) {
        LOGGER.trace(
        ...
        switch (role) {
            ...
            case 'SERVANT':
                ...
                if (index >= 0) {
                    // splice(index) would drop every member from here on
                    list.splice(index, 1);
                    let key = `system.members.${[this.roleMap[role]]}`;
                    await this.update({
                        [key]: list,
    ...
    async addCharacterToHouse(actorId, role, description) {
        ...
        if (role !== 'STEWARD') {
            await this.removeCharacterFromHouse(actorId);
        }
        switch (role) {
            case 'HEAD':
                if (!description) {
